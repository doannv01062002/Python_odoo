...
class TrainingClass(models.Model):
    ...
    total_duration = fields.Integer(string="Thời lượng học (giờ)", help="Tổng thời lượng học của lớp (tính bằng giờ)", required=True, default=0)
    ...
    @api.constrains('total_duration')
    def _check_total_duration(self):
        for record in self:
            if record.total_duration <= 0:
                raise models.ValidationError("Thời lượng học phải lớn hơn 0 giờ.")
            if record.total_duration > 1000:
                raise models.ValidationError("Thời lượng học không được vượt quá 1000 giờ.")
